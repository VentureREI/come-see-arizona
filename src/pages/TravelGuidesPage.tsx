...
const SEASONS = [
  { name: 'Oct - Nov', label: 'Perfect Weather', temp: '75-85°F', vibe: 'Ideal hiking and golf. Sweet spot of pleasant weather and manageable crowds. Hotel rates begin to climb but remain below peak.', color: '#9A3B26' },
  ...
  { name: 'May', label: 'Shoulder Month', temp: '95-105°F', vibe: 'Getting hot but manageable. Significant hotel deals as peak season winds down. Great for pool days.', color: '#D18A2C' },
  { name: 'Jun - Aug', label: 'Summer Heat', temp: '105-115°F', vibe: 'Extreme heat, lowest prices (40-60% off). Plan outdoors before 9am. Escape north to Flagstaff or Prescott.', color: '#A84428' },
...
const body: React.CSSProperties = { fontFamily: 'var(--font-body)', fontSize: 16, lineHeight: 1.8, color: '#4A4239' };
  ...
  return (
    <>
      ...
      <div style={{ background: '#fff', padding: '48px 24px' }}>
        <p style={{ ...body, maxWidth: 900, margin: '0 auto', fontSize: 18, color: '#39322C' }}>
        ...
        <div style={wrap}>
          ...
          <div style={{ display: 'flex', gap: 48, flexWrap: 'wrap' as const }}>
            <div style={{ flex: '1 1 400px', minWidth: 300 }}>
              <h3 style={{ fontFamily: 'var(--font-heading)', fontSize: 20, fontWeight: 600, color: '#26202A', marginBottom: 8 }}>Phoenix Sky Harbor (PHX)</h3>
              ...
              <h3 style={{ fontFamily: 'var(--font-heading)', fontSize: 20, fontWeight: 600, color: '#26202A', marginBottom: 8, marginTop: 24 }}>Tucson International (TUS)</h3>
              ...
              <h3 style={{ fontFamily: 'var(--font-heading)', fontSize: 20, fontWeight: 600, color: '#26202A', marginBottom: 8, marginTop: 24 }}>By Train</h3>
            ...
            <div style={{ flex: '0 1 420px', minWidth: 280 }}>
              ...
              <div style={{ background: '#fff', borderRadius: 12, padding: 24, boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
                <h4 style={{ fontFamily: 'var(--font-heading)', fontSize: 16, fontWeight: 700, marginBottom: 12, color: '#211B24' }}>Driving Distances to Phoenix</h4>
                {[['Los Angeles', '5 hours', 'I-10'], ['Las Vegas', '4.5 hours', 'US-93'], ['San Diego', '5.5 hours', 'I-8'], ['Albuquerque', '6.5 hours', 'I-40']].map(([city, time, route]) => (
                  <div key={city} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid #EFE8DC', fontFamily: 'var(--font-body)', fontSize: 14 }}>
                    <span style={{ color: '#39322C', fontWeight: 500 }}>{city}</span>
                    <span style={{ color: '#8D847A' }}>{time} via {route}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      <div style={{ background: '#fff', padding: '64px 24px' }}>
        <div style={wrap}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 20 }}>
            {[
              { title: 'Rental Car', text: 'Essential for Arizona. The metro spans 50+ miles. Book ahead during peak season (Jan-Apr). Consider an SUV for Sedona or Monument Valley unpaved roads.', accent: '#C1502E' },
              ...
              { title: 'Parking Tips', text: 'Downtown garages $5-15/day. Old Town Scottsdale has metered + garage parking. Popular trailheads: arrive before 7am on weekends during peak season.', accent: '#9A3B26' },
            ].map(item => (
              <div key={item.title} style={{ borderLeft: `4px solid ${item.accent}`, background: '#F7F1E8', padding: '24px 24px', borderRadius: '0 12px 12px 0' }}>
                <h3 style={{ fontFamily: 'var(--font-heading)', fontSize: 18, fontWeight: 600, color: '#211B24', marginBottom: 8 }}>{item.title}</h3>
                <p style={{ fontFamily: 'var(--font-body)', fontSize: 14, lineHeight: 1.6, color: '#5C5348' }}>{item.text}</p>
              </div>
            ))}
          </div>
        </div>
            ...
            {SEASONS.map(s => (
              <div key={s.name} style={{ background: '#fff', borderRadius: 16, overflow: 'hidden', boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
                ...
                <div style={{ padding: '20px 24px' }}>
                  ...
                  <p style={{ fontFamily: 'var(--font-body)', fontSize: 14, lineHeight: 1.6, color: '#5C5348' }}>{s.vibe}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div style={{ background: '#fff', padding: '64px 24px' }}>
        <div style={wrap}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(min(100%, 320px), 1fr))', gap: 16 }}>
            {PACKING.map(p => (
              <div key={p.season} style={{ display: 'flex', gap: 16, background: '#FAF3EB', borderLeft: '4px solid #C1502E', padding: '20px 24px', borderRadius: '0 12px 12px 0' }}>
                <span style={{ fontSize: 28, flexShrink: 0 }}>{p.icon}</span>
                <div>
                  <h3 style={{ fontFamily: 'var(--font-heading)', fontSize: 16, fontWeight: 700, color: '#211B24', marginBottom: 4 }}>{p.season}</h3>
                  <p style={{ fontFamily: 'var(--font-body)', fontSize: 14, lineHeight: 1.6, color: '#5C5348' }}>{p.items}</p>
                </div>
              </div>
            ))}
          </div>
            ...
            {ITINERARIES.map(it => (
              ...
              >
                ...
                <div style={{ padding: 20 }}>
                  <h3 style={{ fontFamily: 'var(--font-heading)', fontSize: 18, fontWeight: 600, color: '#211B24', marginBottom: 8 }}>{it.title}</h3>
                  <p style={{ fontFamily: 'var(--font-body)', fontSize: 14, color: '#5C5348', lineHeight: 1.6 }}>{it.description}</p>
                </div>
              </Link>
            ))}
          </div>
      ...
      <div style={{ background: '#fff', padding: '64px 24px' }}>
        <div style={wrap}>
          ...
          <p style={{ ...body, marginBottom: 32 }}>
            The right neighborhood can make or break your trip. Visit our <Link to="/explore" style={{ color: '#C1502E', fontWeight: 600 }}>Explore Arizona</Link> page for deeper community guides.
          </p>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(min(100%, 340px), 1fr))', gap: 20, marginBottom: 32 }}>
            {[
              ...
              <div key={cat.title} style={{ background: '#fff', borderRadius: 16, padding: 28, boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
                <h3 style={{ fontFamily: 'var(--font-heading)', fontSize: 20, fontWeight: 700, color: '#211B24', marginBottom: 16 }}>{cat.title}</h3>
                {cat.ranges.map(([label, price]) => (
                  <div key={label} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', borderBottom: '1px solid #EFE8DC' }}>
                    <span style={{ fontFamily: 'var(--font-body)', fontSize: 15, color: '#39322C' }}>{label}</span>
                    <span style={{ fontFamily: 'var(--font-body)', fontSize: 15, fontWeight: 600, color: '#C1502E' }}>{price}</span>
                  </div>
                ))}
                <p style={{ fontFamily: 'var(--font-body)', fontSize: 13, color: '#8D847A', marginTop: 12, fontStyle: 'italic' }}>{cat.tip}</p>
              </div>
            ))}
          </div>
          <div style={{ borderLeft: '4px solid #C1502E', background: '#FAF3EB', padding: '24px 28px', borderRadius: '0 12px 12px 0' }}>
            <h3 style={{ fontFamily: 'var(--font-heading)', fontSize: 18, fontWeight: 600, color: '#211B24', marginBottom: 8 }}>Money-Saving Tips</h3>
        ...
        <div style={wrap}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(min(100%, 480px), 1fr))', gap: 24 }}>
            {[
              ...
              <div key={s.season} style={{ background: '#F7F1E8', borderRadius: 16, overflow: 'hidden' }}>
                ...
                <div style={{ padding: 24, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                  <h3 style={{ fontFamily: 'var(--font-heading)', fontSize: 22, fontWeight: 700, color: '#211B24', marginBottom: 8 }}>{s.season}</h3>
                  <p style={{ fontFamily: 'var(--font-body)', fontSize: 14, lineHeight: 1.7, color: '#5C5348' }}>{s.text}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(min(100%, 480px), 1fr))', gap: 32 }}>
            ...
            <div style={{ background: '#fff', borderRadius: 16, overflow: 'hidden', boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
              ...
              <div style={{ padding: 28 }}>
                <h2 style={{ fontFamily: 'var(--font-heading)', fontSize: 24, fontWeight: 700, color: '#211B24', marginBottom: 12 }}>Arizona for Families</h2>
                ...
                <Link to="/things-to-do" style={{ fontFamily: 'var(--font-body)', fontSize: 14, fontWeight: 600, color: '#C1502E', textDecoration: 'none' }}>Explore Things to Do →</Link>
              </div>
            </div>
            {/* Couples */}
            <div style={{ background: '#fff', borderRadius: 16, overflow: 'hidden', boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
              ...
              <div style={{ padding: 28 }}>
                <h2 style={{ fontFamily: 'var(--font-heading)', fontSize: 24, fontWeight: 700, color: '#211B24', marginBottom: 12 }}>Arizona for Couples</h2>
                ...
                <Link to="/where-to-stay" style={{ fontFamily: 'var(--font-body)', fontSize: 14, fontWeight: 600, color: '#C1502E', textDecoration: 'none' }}>Where to Stay →</Link>
              </div>
            </div>
          </div>
        </div>
      ...
      <div style={{ background: '#FAF5F0', padding: '64px 24px' }}>
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          <h2 style={{ fontFamily: 'var(--font-heading)', fontSize: 28, fontWeight: 700, color: '#211B24', marginBottom: 16 }}>Planning to Visit Arizona?</h2>
